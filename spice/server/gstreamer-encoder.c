...
static bool gst_features_lookup(const gchar *feature_name)
{
    ...
    //         spice_warning ("GStreamer init failed: %s", err->message);
    //         g_clear_error(&err);
    //         return false;
    //     }

    //     GList *l = gst_element_factory_list_get_elements(
    //              GST_ELEMENT_FACTORY_TYPE_VIDEO_ENCODER,
    //              GST_RANK_MARGINAL);
    //     spice_debug("=== VIDEO_ENCODER factories ===");
    //     for (; l; l = l->next) {
    ...
    //     /* 2a) Either load the single VA-API plugin by absolute path: */
    //     if (!gst_plugin_load_file ("/usr/lib64/gstreamer-1.0/libgstva.so",
    //                                &err)) {
    //         spice_warning ("Failed to load libgstva.so: %s", err->message);
    //         g_clear_error(&err);
    //         /* we continue, maybe it’s already loaded elsewhere */
    //     }

    //     done_init = true;
    // }

    /* 3) Now lookup the factory */
    GstElementFactory *f = gst_element_factory_find (feature_name);
    if (!f) {
        spice_debug ("factory '%s' not found", feature_name);
        return false;
    }
    gst_object_unref (f);
    return true;
...
static gchar *find_best_hw_plugin(const gchar *codec_name)
{
	...
	/* Realistically we don't need to search more than 2 render nodes ever but we will
	/  search more a just in case and b for futureproofing so we can add code here where
	/  it will collect a bunch of possible ones i.e. like 5 lpencs and choose one at
	/  random for multigpu offloading and balancing. */

    // If we're just looking for postproc, check base plugins first and return early
    if (!codec_name) {
        for (i = 0; i < G_N_ELEMENTS(plugins); i++) {
            feature_name = g_strconcat(plugins[i], "postproc", NULL);
    ...
    // Try VA low power encoder first
    feature_name = g_strconcat("va", codec_name, "lpenc", NULL);
    if (gst_features_lookup(feature_name)) {
        return feature_name;
    }
    g_free(feature_name);

	// Then try renderD nodes (128-135) with low power
    for (i = 128; i <= 135; i++) {
        feature_name = g_strconcat("varenderD", g_strdup_printf("%d", i), codec_name, "lpenc", NULL);
        if (gst_features_lookup(feature_name)) {
            return feature_name;
        }
        g_free(feature_name);
	...
	// Then try VA regular encoder
    feature_name = g_strconcat("va", codec_name, "enc", NULL);
    if (gst_features_lookup(feature_name)) {
        return feature_name;
    }
    g_free(feature_name);

	// Then try renderD nodes (128-135) with regular encoder
    for (i = 128; i <= 135; i++) {
        feature_name = g_strconcat("varenderD", g_strdup_printf("%d", i), codec_name, "enc", NULL);
        if (gst_features_lookup(feature_name)) {
            return feature_name;
        }
        g_free(feature_name);
	...
	// Finally try msdk and vaapi
    for (i = 0; i < G_N_ELEMENTS(plugins); i++) {
        if (strcmp(plugins[i], "va") == 0) {
            continue;  // Skip VA as we already tried it
        }
...
static gchar *get_hw_gstenc_opts(const gchar *encoder, const gchar *codec_name)
{
	...
	// Depricate VP9 specific code (it's too ancient)
    if (g_str_has_prefix(encoder, "msdk")) {
		...
		// Codec specific options, we can exclude them from above hopefully as most thing will be VA
		if (strcmp(codec_name, "av1") == 0) {
			gstenc_opts = g_strdup("rate-control=4 target-usage=7 min-qp=15 max-qp=35");
		}
		else if (strcmp(codec_name, "h265") == 0) {
			gstenc_opts = g_strdup("rate-control=4 b-frames=0 target-usage=7 min-qp=15 max-qp=35");
		}
		else {
                                 ...
                                 gchar **gstenc_name, gchar **gstenc_opts)
{
    ...
    spice_debug("Trying intel HW Plugin");

    if (strcmp(codec_name, "vp8") == 0) {
        return;
    }

	// VP9 is also legacy at this point on VA-API
	if (strcmp(codec_name, "vp9") == 0) {
        return;
    }

    spice_debug("Finding best hardware plugin");
    ...
    spice_debug("Finding best hardware plugin with NULL");
...
static gboolean create_pipeline(SpiceGstEncoder *encoder)
{
    ...
    gchar* gstenc_opts;
	/* These are specific options for the software encoders i.e. vp8/9, x264, x265, av1
	/  they will get overwritten at get_hw_gstenc_opts for the VA-API encoders so options
	/  not matching is ok */
    switch (encoder->base.codec_type)
    {
    case SPICE_VIDEO_CODEC_TYPE_MJPEG:
    ...
    case SPICE_VIDEO_CODEC_TYPE_H264:
        ...
        break;
	case SPICE_VIDEO_CODEC_TYPE_H265:
		/* Same options as x264 with some changes
		/  - x265 doesn't have aud because thats legit an improvement over x264
		/  - x265 doesn't have qp min and maxes because the encoder is ranged by
		/  	 default therefore we just choose an optimal qp for livestreaming
		/  - x265 doesn't have sliced threads well it hopefully will be fine with frame based
		/  - x265 should be intra-PERIODIC-refresh by default
		*/
        gstenc_opts = g_strdup("qp=25 tune=4 speed-preset=ultrafast");
        break;
	case SPICE_VIDEO_CODEC_TYPE_AV1:
		/* Well this is going to be fun
		/  - cpu-used set to 10 so we get much lower latency at cost of quality
		/  - drop-frame realistically we want it to drop frames if it can't keep up
		/  - rest can be autodetected by usage-profile
		*/
		gstenc_opts = g_strdup("cpu-used=8 drop-frame=100 usage-profile=1 resize-mode=1");
        break;
    default:
        /* gstreamer_encoder_new() should have rejected this codec type */
        spice_warning("unsupported codec type %d", encoder->base.codec_type);
    ...
    spice_debug("Codec Name: %s", codec_name);

    GpuVendor intel_vendor = spice_udev_detect_gpu(INTEL_VENDOR_ID);
    // AMD Can also do VA-API plugins
    GpuVendor amd_vendor = spice_udev_detect_gpu(AMD_VENDOR_ID);

    if (intel_vendor == VENDOR_GPU_DETECTED || amd_vendor == VENDOR_GPU_DETECTED) {
        spice_debug("GPU vendor detected");
...
static void set_gstenc_bitrate(SpiceGstEncoder *encoder)
{
    ...
    /* This might be the funniest bug fix I had to do in a long time and also the
    /  funniest hard code ever, it basically checks if the description has kbit
    /  howerver av1 uses kilobits lol, also the VA-API use kbps so also bugfix! */
    if (strstr(g_param_spec_get_blurb(param), "kbit") ||
        strstr(g_param_spec_get_blurb(param), "kbps") ||
        strstr(g_param_spec_get_blurb(param), "kilobits")) {
