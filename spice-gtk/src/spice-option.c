...
gboolean disable_clipboard = FALSE;
...
{
    if (disable_clipboard && SPICE_IS_MAIN_CHANNEL(channel)) {
        g_signal_handlers_disconnect_by_func(channel, NULL, G_CALLBACK(spice_main_channel_clipboard_selection_grab));
        g_signal_handlers_disconnect_by_func(channel, NULL, G_CALLBACK(spice_main_channel_clipboard_selection_request));
        g_signal_handlers_disconnect_by_func(channel, NULL, G_CALLBACK(spice_main_channel_clipboard_selection_notify));
        g_signal_handlers_disconnect_by_func(channel, NULL, G_CALLBACK(spice_main_channel_clipboard_selection_release));
        
        g_print("Clipboard sharing disabled - all clipboard signals disconnected\n");
    }
}
...
GOptionGroup* spice_get_option_group(void)
{
    const GOptionEntry entries[] = {
        ...
        { "spice-disable-clipboard", '\0', 0, G_OPTION_ARG_NONE, &disable_clipboard,
            N_("Disable clipboard sharing between client and guest"), NULL },
...
void spice_set_session_option(SpiceSession *session)
{
    ...
    if (disable_clipboard) {
        g_signal_connect(session, "channel-new", G_CALLBACK(on_channel_new), NULL);
    }
}
